...
function Shortlisted() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch('/final-shortlisted-result.json');
    ...
    const handleCheckResult = () => {
      ...
      const { passedApplicants, restrictedApplicants, message, link } = cityData;
      if (passedApplicants.includes(Number(phoneNumber))) {
        setResult(message);
        setMessageTextColor('text-[#0d6efd]');
        setLink(link); // Set the link if it exists
    ...
    return (
      ...
      >
        ...
        <div className="w-full flex flex-col items-center z-10 res-result-main">
          ...
          >
            {link ? (
              <a href={link} className={`font-bold mb-[10px] ${messageTextColor} underline`}>
                <p>{result}</p>
              </a>
            ) : (
              <p className={`font-bold mb-[10px] ${messageTextColor}`}>{result}</p>
            )}
            <p className="font-bold text-[14px] mb-[3px] text-dark-color">Appeared in test but can't find result? Submit your query regarding result only we will respond you as soon as possible.</p>
            ...
            <p className="font-bold text-[14px] text-dark-color">&copy; Labbaik Talent Test Result</p>
